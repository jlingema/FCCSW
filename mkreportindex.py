...
def get_diff_map(file_name):
    ...
    diff_lines = common._ComparisonReport(file_name, clang_format_output)
    if num_violations != 0:
        return diff_lines
    else:
        return False
...
if __name__ == "__main__":
    ...
    error_dict, total_errors = write_report_files(base_dir, report_dir)
    ...
    with open(os.path.join(report_dir, "index.html"), "w") as fobj:
        fobj.write(report_template.format(links="\n".join(substrings), title=os.path.basename(base_dir), n=total_errors))
    print("--------------------------------------------------------------------------------")
    print("Check Complete\nNumber of errors found: {total_errors}\nReport index at: {index_path}".format(total_errors=total_errors, index_path=os.path.abspath(os.path.join(report_dir, "index.html"))))
    print("--------------------------------------------------------------------------------")