...
MomentumRangeParticleGun::MomentumRangeParticleGun( const std::string & type ,
  ...
  declareProperty( "EtaMin"       , m_minEta = -99                      , "Minimal eta"       ) ;
  ...
  declareProperty( "EtaMax"      , m_maxEta   = -99                                    , "Maximal eta"      ) ;
...
StatusCode MomentumRangeParticleGun::initialize() {
  ...
  // check eta and theta:
  if (m_minEta != -99 || m_maxEta != -99) {
    info() << "You specified an eta value which will override any theta value you specified!" << endmsg;
    m_minTheta = 2*atan(exp(-m_minEta)) * Gaudi::Units::rad;
    m_maxTheta = 2*atan(exp(-m_maxEta)) * Gaudi::Units::rad;
  }
  ...
  info() << " -- corresponding Eta range: " << m_minEta << " <-> "
   << m_maxEta << " rad" << endmsg ;
